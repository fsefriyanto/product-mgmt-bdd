...
public class ProductOrderSD implements En {
	...
	public ProductOrderSD(World world) {
		this.world = world;

		When("^user add new product order with following detail$", (DataTable dataTable) -> {

			List<List<String>> data = dataTable.raw();

			Map<String, Object> productOrder = new HashMap<>();
			productOrder.put("category", data.get(0).get(1));
			productOrder.put("customerOrderType", data.get(1).get(1));
			...
			productOrder.put("productOfferings", prodOffers);
			...
			productOrder.put("customers", customers);

			response = request.given().contentType(ContentType.JSON).body(productOrder).when().post("/product/order")
		...
		Then("^new product order is created$", () -> {
			...
			this.world.productOrderId = jsonPath.get("id").toString();
		});
	}
